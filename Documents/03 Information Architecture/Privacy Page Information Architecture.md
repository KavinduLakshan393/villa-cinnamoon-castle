...
The public copy was refined during implementation under DEC-019 (2026-09-23). Internal wording such as `Phase 1` and `should` was replaced with plain statements that describe the website's verified behaviour. The previous version is preserved in `../99 Archive/Privacy Page Information Architecture - Pre-Implementation 2026-09-23.md`.
...
- Self-host all web fonts and scripts. A network audit on 2026-09-23 confirmed that no page requests any third-party origin and that the site sets no cookies or local storage. The only browser storage is the session entry `vcc-inquiry`. Repeat this audit before launch.
